...
class HomeController extends Controller
{
    ...
    public function index() {
        ...
        /*get slider images*/
        $SliderDetails 		=  Sliders::select('id','title','sliderImage','description','link','status')->where('status','=','active')->get();   
		...
		$Categories 		= Categories::join('subcategories', 'categories.id', '=', 'subcategories.category_id')
								...
								->where('categories.status','=','active')->get()->toArray();  
		...
		$PopularProducts 	= Products::join('order_products', 'products.id', '=', 'order_products.product_id')
							  ->select('products.*',DB::raw("count(order_products.id) as totalOrderedProducts"))
