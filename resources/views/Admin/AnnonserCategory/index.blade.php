    ...
    <div class="col-12">
      ...
      <div class="card">
        <div class="card-header">
          <h4>{{ __('users.annonser_category_list')}}</h4>
           <a href="{{route('adminAnnonserCatCreate')}}" title="{{ __('users.add_category_btn')}}" class="btn btn-icon btn-success" style="margin-left:542px;"><span>{{ __('users.add_category_btn')}}</span> </a>
        </div>
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h4 class="modal-title">{{ __('users.add_annonser_subcategory')}}</h4>
          <button type="button" class="close" data-dismiss="modal">&times;</button>
        </div>
        
        <div class="modal-body">
            <div class="container">
            <form action="{{route('adminAnnonserSubCatStore')}}"  enctype="multipart/form-data" method="post" class="savecategoryform">
              @csrf
...
<script type="text/javascript">
    ...
    $(document).on("click",".savesubcategorydata",function(event) {
          ...
          $.ajax({
          url: "{{url('/')}}"+'/admin/AnnonserSubcategory/check-unique-subcat/?subcat_name='+subcatname,
          type: 'get',
          async: false,
          data: { },
     ...
     var dataTable = $('#categoryTable').DataTable({
        ...
        "ajax": {
          ...
          url : '{{route("adminAnnonserCatGetRecords")}}',
          'data': function(data){
...
/*function to check unique Slug name
  ...
  */
  function checkUniqueSlugName(inputText){
    ...
    var slug;
     $.ajax({
      url: "{{url('/')}}"+'/admin/AnnonserSubcategory/check-slugname/?slug_name='+slug_name,
      type: 'get',
      async: false,
      data: { },
      success: function(output){
