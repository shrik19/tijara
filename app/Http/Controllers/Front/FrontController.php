			...
			if(strpos(@$request->path, 'annonser') !== false){
				...
				$Products 			= Products::leftjoin('category_products', 'products.id', '=', 'category_products.product_id')
							  ...
							  ->where(function($q) use ($currentDate) {
								...
								$q->where([["users.role_id",'=',"2"],['user_packages.payment_status','=','CAPTURED'],['user_packages.status','=','active'],['start_date','<=',$currentDate],['end_date','>=',$currentDate],['variant_product.quantity', '>', 0]])->orWhere([["users.role_id",'=',"1"],['is_sold','=','0'],[DB::raw("DATEDIFF('".$currentDate."', products.created_at)"),'<=', 30]])
...
//echo "in";exit;
    ...
    public function sendServiceRequest(Request $request)
    {
		...
		$siteLogo     = url('/')."/uploads/Images/white_logo.png";
