					  ...
					  <tbody>
					    ...
					    <tr>
					    	...
					    	@if($row->start_date >= date('Y-m-d H:i:s') && $row->payment_status=="CAPTURED"  )
					  		...
					  		@elseif(($row->start_date >= date('Y-m-d H:i:s') && $row->payment_status=="checkout_incomplete") || $row->status == "block")
					  		<td><a href="javascript:void(0)" class="btn btn-danger"> {{ __('lang.pending_label')}}</a>
					  			...
					  			<a href="javascript:void(0)" class="btn btn-info"> {{ __('lang.pending_label')}}Reload</a></td>
					  		@else
