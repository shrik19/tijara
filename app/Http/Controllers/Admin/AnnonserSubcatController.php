...
class AnnonserSubcatController extends Controller
{
    /*
    ...
    public function index(Request $request) {
        $data = [];
        $data['pageTitle']              = trans('users.annonser_subcategory_title');
        $data['current_module_name']    = trans('users.annonser_subcategory_title');
        $data['current_id']             = $request->id;
        $data['module_name']            = trans('users.annonser_subcategory_title');
        $data['module_url']             = route('adminAnnonserSubcat',$request->id);
        $data['recordsTotal']           = 0;
        $data['currentModule']          = '';

        return view('Admin/AnnonserSubcategory/index', $data);
    }
    ...
    public function getRecords(Request $request) {
		...
		$categoryDetails = AnnonserCategories::select('annonsercategories.category_name')->where('id','=',$category_id)->get()->first();

		$SubcategoryDetails = AnnonserSubcategories::select('annonserSubcategories.*')->where('category_id','=',$category_id);



		if(!empty($request['search']['value']))
        {
            $SubcategoryDetails = $SubcategoryDetails->where('annonserSubcategories.subcategory_name', 'LIKE', '%'.$request['search']['value'].'%');

        }

        if (!empty($request['status']) && !empty($request['search']['value'])) {
            $SubcategoryDetails = $SubcategoryDetails->where('annonserSubcategories.status', '=', $request['status']);
        }
        ...
        if (!empty($request['order'])) {
            $column = 'annonserSubcategories.id';
            $order = 'desc';
            $order_arr = [
                            '0' =>  'annonserSubcategories.id',
                            '1' =>  'annonserSubcategories.category_id',
                            '2' =>  'annonserSubcategories.subcategory_name',
                            '3' =>  'annonserSubcategories.sequence_no',
                         ];
        ...
        if (count($recordDetails) > 0) {
            ...
            foreach ($recordDetails as $recordDetailsKey => $recordDetailsVal)
			{
                ...
                if ($recordDetailsVal['status'] == 'active') {
                    $status = '<a href="javascript:void(0)" onclick=" return ConfirmStatusFunction(\''.route('adminAnnonserSubcatChangeStatus', [base64_encode($recordDetailsVal['id']), 'block']).'\');" class="btn btn-icon btn-success" title="'.__('lang.block_label').'"><i class="fa fa-unlock"></i> </a>';
                } else {
                    $status = '<a href="javascript:void(0)" onclick=" return ConfirmStatusFunction(\''.route('adminAnnonserSubcatChangeStatus', [base64_encode($recordDetailsVal['id']), 'active']).'\');" class="btn btn-icon btn-danger" title="'.__('lang.active_label').'"><i class="fa fa-lock"></i> </a>';
                }

                $action = '<a href="'.route('adminAnnonserSubCatEdit', base64_encode($id)).'" title="'.__('users.edit_title').'" class="btn btn-icon btn-success"><i class="fas fa-edit"></i> </a>&nbsp;&nbsp;';

                $action .= '<a href="javascript:void(0)" onclick=" return ConfirmDeleteFunction(\''.route('adminAnnonserSubcatDelete', base64_encode($id)).'\');"  title="'.__('lang.delete_title').'" class="btn btn-icon btn-danger"><i class="fas fa-trash"></i></a>';
    ...
    public function subCategoryStore(Request $request) {
        ...
        $rules = [
            ...
            'subcategory_name' => 'required|unique:annonserSubcategories,subcategory_name',
        ...
        if($validator->fails())
        {
            ...
            /*if(!empty($id)){
                ...
                AnnonserSubcategories::where('id', '=', $id)->update($arrUpdate);
                ...
                AnnonserSubcategories::create($arrInsertSubcategory);
    ...
    public function edit($id) {
       if(empty($id)) {
        ...
        $details = AnnonserSubcategories::where('annonserSubcategories.id', $id)
        ->Join('annonsercategories', 'annonserSubcategories.category_id', '=', 'annonsercategories.id')
        ->select('annonserSubcategories.*','annonsercategories.id','annonsercategories.category_name')->first();
        ...
        $data['module_url']             = route('adminAnnonserSubcat',$details['id']);
        $data['categoryDetails']        = $details;


        return view('Admin/AnnonserSubcategory/edit', $data);
    }
    ...
    public function update(Request $request, $id) {
        ...
        $slug =   CommonLibrary::php_cleanAccents($subcategory_slug);
        $rules = [
            'subcategory_name' => 'required|unique:annonserSubcategories,subcategory_name,'.$id,
            'sequence_no'      => 'required',
        ...
        AnnonserSubcategories::where('id', '=', $id)->update($arrUpdateCategory);

        Session::flash('success', trans('messages.subcat_update_success'));
        return redirect(route('adminAnnonserSubcat',base64_encode($request->input('hid_subCategory'))));
    }
    ...
    public function delete($id) {
        if(empty($id)) {
            Session::flash('error', trans('errors.refresh_your_page_err'));
            return redirect(route('adminAnnonserSubcat'));
        }
        $id = base64_decode($id);

        $categories = AnnonserSubcategories::find($id);
        if (!empty($categories))
		{
    ...
    public function changeStatus($id, $status) {
        ...
        $result = AnnonserSubcategories::where('id', $id)->update(['status' => $status]);
        if ($result) {
    ...
    */
    function checkUniqueSlugName(Request $request){
        ...
        if(!empty($id)){
            $data =  AnnonserSubcategories::where('subcategory_slug', $slug)->where('id','!=',$id)->get();
        } else{
            $data =  AnnonserSubcategories::where('subcategory_slug', $slug)->get();
        }
    ...
    */
    function checkUniqueSubcat(Request $request){
        ...
        $id = $request->id;
        if(!empty($id)){
            $data =  AnnonserSubcategories::where('subcategory_name', $subcat_name)->where('id','!=',$id)->get();

        } else{
            $data =  AnnonserSubcategories::where('subcategory_name', $subcat_name)->get();
        }
